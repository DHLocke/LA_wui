...
TODO combine land cover summaries to buildings (from new analyses)
TODO bring distance shrub


TODO 'final' double checks

TODO fit models suggested by Alex.
...
### 2b building distance veg
```{r}

(min_dist_shrub <- read_csv('output_data/min_dist_shrub_2021-03-16.csv'))
(min_dist_tree  <- read_csv('output_data/min_dist_tree_2021-03-16.csv'))
...
### 2d ALT building land cover buffer
```{r}
...
### 2e double check land cover values
```{r}
# build %>% mapview(., zcol = 'build_p_tree_10')
...
build %>% ggplot(aes(min_dist_tree)) + geom_histogram(binwidth = 2)
build %>% ggplot(aes(min_dist_shrub)) + geom_histogram(binwidth = 2)
...
system.time(
build %<>% st_join(st_make_valid(parcel), # spatial join
...
box_plot_dat <- build %>% 
  ...
  select(damage_cat,
         starts_with('build_'),
         starts_with('parcel_') &
         !parcel_AIN & !parcel_UseType & !parcel_UseDescription &
           -contains('Year')) %>% data.frame %>% glimpse
