...
#need to merge with build at end
...
subdins<-dins %>% 
  ...
  mutate_at(vars(c(2:12)), list(~ ifelse( . == "Unknown", NA, .)))%>% 
  mutate_at(vars(c(2:12)), list(~ ifelse( . == " ", NA, .)))%>% 
  mutate_at(vars(c(2:12)), list(~ ifelse( . == "N/A", "None", .)))


#Here here here; need to order these 
