...
# # destroyed and building exposed, 11,134 observations
...
min_dest2<- read_csv('data/Near_dest220131_2.csv') %>%  #distance to destroyed building;
  ...
  dplyr::select(c(ID_Build=Buildings_ParcelID_DINS_ID_Build,near_dest=NEAR_DIST,Rank=NEAR_RANK)) %>%
  ...
  ungroup()%>% dplyr::select(ID_Build,near_dest) %>% 
  distinct()
#  
# # older processing on min_dest2 to check number of duplicates
# min_dest2 %>% duplicated() %>% tabyl() #11140 are not duplicates
...
min_exp2<- read_csv('data/Near_exp220131_2.csv') %>%  #distance to exposed building;
  ...
  dplyr::select(c(ID_Build=Buildings_ParcelID_DINS_ID_Build,near_exp=NEAR_DIST,Rank=NEAR_RANK)) %>%
      filter(near_exp!=0) %>%
  group_by(ID_Build) %>%
  filter(near_exp == min(near_exp)) %>%
  ungroup()%>% dplyr::select(ID_Build,near_exp)%>% 
  distinct()
...
min_build<- read_csv('data/near_allbuild220131.csv') %>%  #distance to destroyed building;
  # if this is 0, it is distance to self, delete
  dplyr::select(c(ID_Build=Buildings_ParcelID_DINS_ID_Build,dist_build=NEAR_DIST,Rank=NEAR_RANK)) %>%
      filter(dist_build!=0) %>%
  group_by(ID_Build) %>%
  filter(dist_build == min(dist_build)) %>%
  ungroup()%>% dplyr::select(ID_Build,dist_build)%>% 
  distinct()
...
table(build$build_near_exp>=build$build_near_build)
plot(build$build_near_build,build$build_near_exp) #does this seem right? #Dexter 

#different building distance measures, #Dexter - talk next steps
table(build$build_near_exp<build$build_dist_to_build_ft_jod) #Dexter - how can build_near_exp be smaller than distance to another building?
table(build$build_near_build==build$build_dist_to_build_ft_jod)
test<-build %>% 
filter(build$build_near_exp<build$build_dist_to_build_ft_jod) %>%
  tidylog::select(ID_Build,build_dist_to_build_ft_jod,build_near_dest,build_near_exp)
