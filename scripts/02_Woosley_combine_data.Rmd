...
DHL this means 70% have a value of 1. A boxplot for a binary variable is not appropriate if the other variable is also binary
...
### a adding in building distance to destroyed and exposed by fire
```{r} 
...
min_dist_dest<- read_csv('data/Near_dest210922.csv')  #distance to destroyed building; if this is 0, it is distance to self, value 2
min_dist_exp<- read_csv('data/Near_exp210922.csv')  #distance to exposed building; if this is 0, it is distance to self,  value 2
 
#processing to get to min distance to destroyed
min_dest<- min_dist_dest %>% dplyr::select(c(IN_FID,Dist=NEAR_DIST,Rank=NEAR_RANK)) %>%
...
min_exp<- min_dist_exp %>% dplyr::select(c(IN_FID,Dist=NEAR_DIST,Rank=NEAR_RANK)) %>%
...
cor(min_dest$near_dest, min_exp$near_exp, method=c("pearson")) #highly correlated
