...
# get building distanct to shrub
# get building distanct to tree
...
# buffer
tic(); build_10  <- build %>% st_buffer(., dist = 10); 
...
# initialize lists
donut_300 <-donut_200<-donut_100<-donut_10<-rep(list(data.frame(ID_Build  = NA_integer_)), nrow(build))
...
tic();donut_10  %<>% bind_rows()
donut_100 %<>% bind_rows() 
donut_200 %<>% bind_rows() 
donut_300 %<>% bind_rows(); toc()


#mapview(donut_300[1:10,]) + mapview(donut_200[1:10,]) +
  mapview(donut_100[1:10,]) + mapview(donut_10[1:10,])


mapview(donut_100[50:110,]) + mapview(donut_10[50:110,])

#build %>% slice(1:50) %>% mapview() + build_10 %>% slice(1:50) %>% mapview()

# optional clean up?
rm(build_10, build_100, build_200, build_300)
```


## 3 try some land cover summaries
### donut 10
```{r}
...
(time_12_cores_10ft_buff <- proc.time() - start_12_cores)
...
#                 a_total) %>% # just cosmetic reordering
  mutate(p_tree        = 100*(a_tree        / a_total),
         ...
         p_other_paved = 100*(a_other_paved / a_total),
         p_shrub       = 100*(a_shrub       / a_total),
         `Impervious Surface (%)` = 100*((a_building + a_road + a_other_paved) /
                                      (a_tree + a_grass + a_soil + a_building + a_road + a_other_paved))) %>% 
...
(time_12_cores_100ft_buff <- proc.time() - start_12_cores)
...
(time_12_cores_200ft_buff <- proc.time() - start_12_cores)
...
(time_12_cores_300ft_buff <- proc.time() - start_12_cores)
