...
#Dexter, any thoughts on build_min_dist_tree - and INF?
# YES - it sux and I need to fix it, ha!. I put a note in 02_Woosle...
```
...
# problem build_min_dist_tree

build %>% map(., ~sum(is.na(.)))
build %>% 
  select_if(is_double) %>% # remove non-numerically coded variables
  tidylog::select(build_Mean_elev_30m : parcel_area) %>% # remove vars coded as numeric but not
  tidylog::select(-parcel_Roll_HomeOwnersExemp, -build_has_tree_overhang) %>%   # like damage_binary, IDs stored as numbers
  cor() %>% View()
