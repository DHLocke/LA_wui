...
build <- read_csv(paste0(getwd(), '/output_data/building_2021-09-22.csv')) %>% glimpse
```
...
box_plot_dat <- build %>%
  ...
  dplyr::select(c(damage_binary,build_Mean_elev_30m:build_shrub_300,parcel_Roll_ImpValue.y:parcel_Imperv_P,!parcel_AIN)) %>%  #this might need to be revised if build variables change
  data.frame()
