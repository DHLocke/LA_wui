...
# FIXME slice is really dangerous here - just be careful and double, tripple check you got what you wanted
...
tbl_DEF <- build %>% 
  ...
  add_p() #%>%  
  # as_gt() # %>% 
 # modify_header(label == "**Defensible Space**")  #Dexter, I can't get this label to change - I want characteristic to be 'defensible space' instead
...
tbl_LAND <- build %>% 
  ...
  add_p() #%>%  
  # as_gt()  #%>% 
 # modify_header(label = "**Landscape Attributes**") #Dexter, I can't get this label to change - I want characteristic to be 'landscape attributes' instead

tbl_desc <- tbl_stack(list(tbl_DEF, tbl_LAND)
                      , group_header = c("**Defensible Space**", "**Landscape Attributes**")
                      ) # as_gt() in the two pieces above were the problem. Also, tbl_stack() has a header
# option so I put those lables in there (instead of in each table when they are made)

# save out
tbl_desc |> 
  as_gt() |> # looks like as_gt() IS needed, but down here. 
  gt::gtsave(paste0(getwd(), '/figures/Table_Descript_' , Sys.Date(), '.png'))

# TODO change order to be grouped by theme. 
build %>%  
  select(-damage_severity, -build_DINS) %>%  # to be all-inclusive
  tbl_summary(by = damage_cat) %>%  
  add_overall() %>%  
  add_p() %>%  
  as_gt() %>% 
  gt::gtsave(paste0(getwd(), '/figures/Table_Descript_ALL_VARS' , Sys.Date(), '.html'))



#IS NUMERIC PROBLEM
str(build$build_p_water_10)
is.numeric(build$build_p_building_10)
is.numeric(build$build_p_water_10)
