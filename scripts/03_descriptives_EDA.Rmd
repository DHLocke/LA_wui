...
#The below is me exploring from https://drsimonj.svbtle.com/exploring-correlations-in-r-with-corrr
#Not complete however - looks like the highest measures are when parcel and landcover by building are compared
#build_has_tree_overhang  ; build_overhang_ht  ;build_perc_overhang - which of these?
 
build %>%
  tidylog::select_if(is_double) %>% 
    corrr::correlate()->rs
any_over_70 <- function(x) any(x > .7, na.rm = TRUE)
rs %>% select_if(any_over_70)->rs.1
View(rs.1)

#widyr - does correlation where one column is variable one and then let's you pull out the pairs.
