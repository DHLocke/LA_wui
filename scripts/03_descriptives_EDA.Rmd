...
build %>% 
  select_if(is_double) %>% 
  select(damage_binary : parcel_area) %>% 
  describe %>% 
  data.frame() %>% 
...
# TODO final decision on sample size 11,134 or fewer?
...
## 3 correlations
```{r}
plot(build$build_Mean_distroad, build$build_Mean_builddens)

# where are the nulls
build %>% map(., ~sum(is.na(.)))

build %>%
  select(build_Mean_elev_30m : build_p_shrub_300,
         parcel_Roll_ImpValue : parcel_area
         ) %>%
  tab_corr(., triangle = 'bottom', file = paste0(getwd(), '/output_data/correlations_',
   gsub('[[:punct:]]', '_', Sys.time()), '.html'))

#TODO what is build_dist_to_build_ft_jod? why not same as ours? sniff
#TODO build_min_dist_tree is the same as build_min_dist_shrub
#TODO add in building footprint area
...
```{r}
build %>% map(., ~sum(is.na(.)))
#Dexter, this is super helpful - so, a bunch of missing values still? TODO -decide on smaller dataset or see if it even matters
```
...
# build %>% 
#   tidylog::select(ID_Build, starts_with("parcel_Effective"), parcel_year_built,
