...
## 1 read in the data
```{r}
build <- read_csv(paste0(getwd(), '/output_data/building_2021-04-06.csv'))
```
...
```{r}
build %>% describe #%>%
  # write_csv(., paste0(getwd(), '/output_data/describe_',               #TODO doesn't have variable names, not a big deal but maybe want to change?
  #                     gsub('[[:punct:]]', '_', Sys.time()), '.csv'))   #TODO final decision on sample size 11,134 or fewer?
...
## 3 correlations
```{r}
# all #TODO this output isn't working
build %>% tidylog::select(build_Mean_elev_30m : parcel_year_built) %>% 
