...
test<-build %>%  filter(build_area<400) 
View(test)
summary(test$build_area)
hist(test$build_area)
...
(tab_labs |> filter(Table2 == 1) |> pull(`Variable name`) -> vars_to_include_tbl_2)


# TODO change order to be grouped by theme. 
build |> 
  # select(-damage_severity, -build_DINS) |> # to be all-inclusive
  # select(  damage_cat                     # to match Miranda's selections above re: defensible space and over hang
  #        , parcel_area
  #        , build_area
  #        , build_min_dist_tree
  #        , build_near_build
  ...
  #        , build_overhang_ht
  #        ) |> 
  select(damage_cat, any_of(vars_to_include_tbl_2)) |> 
