...
# cors_tibble |> 
#   write_csv(
#     paste0(getwd(), '/output_data/correlations_',
...
# table for paper with only pruned variables
# Feb 23 2023
# full_table <-
#      read_csv(paste0(getwd(), '/data/Table_Deviance_Explained_2023-02-15.csv'))  %>%  #latest deviance explained
#     left_join(vars_to_include %>% #add in variables to include
#            mutate (small_table = 1) %>%
#                 select(name, small_table), by = c('Variable name' = 'name')) %>% #join and retain all
#             mutate ( small_table = if_else (Source == 'Structure assessment-DINS', 1, small_table)) #recode so that all DINS materials data are retained for small table
# # 
# # #export this back out
# full_table %>%
#   write_csv(paste0(getwd(), '/data/Full_Table_Deviance_Explained_', Sys.Date(), '.csv'))
#             
```
...
# ggsave(paste0(getwd(), '/figures/roc_all_', Sys.Date(), '.png')
#        , width = 6.5, height = 6.5)
...
tab_labs2 <- 
# read_csv(paste0(getwd(), '/data/Table_Deviance_Explained_2022-06-13.csv'))
  read_csv(paste0(getwd(), '/data/Table_Deviance_Explained_2023-02-15.csv'))
...
variable_importance %>% 
  ...
  mutate (Description = Label,
          Theme = Type) %>% 
  tidylog::select(Variable, Model, Theme)  -> top_pdp

tab_labs2 %>% 
  rename(Dev= "Deviance Explained (%)") %>% #was not sorting without shorter name
  arrange(desc(Dev)) %>% 
  mutate(Model = "Univariate") %>%  
  rename(Variable= 'Variable name')  %>% 
  slice(1:top_n) %>% 
  tidylog::select(Variable, Model, Theme) ->top_uni

bind_rows(top_pdp, top_uni) -> top_vars

View(top_vars)
table(top_vars$Theme) #checking on, types are all correct
...
top_vars %>% 
  ...
  tidylog::select(Theme, Variable, Model) %>% # reorder to match example below
  group_by(Variable) %>%
  ...
  arrange(Theme,desc(count)) ->top_vars_group

top_vars_group %>%
   write_csv(., paste0(getwd(), '/output_data/top_vars_group_', Sys.Date(), '.csv')) # 
