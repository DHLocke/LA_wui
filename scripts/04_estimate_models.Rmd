...
#             mutate ( small_table = if_else (Theme == 'CON', 1, small_table)) #recode so that all CON (DINS materials data) are retained for small table
# 
...
variable_importance %>% 
  ...
  mutate (Description = Label,
          Theme = Type) %>% 
  tidylog::select(Variable, Model, Description, Theme)  -> top_pdp

tab_labs2 %>% 
  rename(Dev= "Deviance Explained (%)") %>% 
  arrange(desc(Dev)) ->test

%>% 
  mutate(Model = "Univariate") %>%  
  rename(Variable= 'Variable name')  %>% 
  slice(1:top_n) %>% 
  tidylog::select(Variable, Model, Description, Theme) ->top_uni
