...
#             , file = paste0('../Massive_forests/cf_dins_binary_', Sys.Date(), '.Rdata')); toc() # ~20 secs

# # read back in
# tic(); load(file = '../Massive_forests/cf_dins_2022-02-22.Rdata'); toc() # 1.5 seconds
# tic(); load(file = '../Massive_forests/cf_dins_binary_2022-05-17.Rdata'); toc() # 4 seconds 2000 trees
# tic(); load(file = '../Massive_forests/cf_dins_binary_2022-05-20.Rdata'); toc() # <1 second with 500 trees
...
(
  variable_importance <- read_csv(paste0(getwd()
    ...
    ungroup() |> 
    left_join(
      tab_labs |> tidylog::select(Variable = `Variable name`, Alias, Type, Scale)
      , by = 'Variable') |> 
...
pdp::partial(cf_no_dins               # the model
 ...
 stopCluster(cl); toc(); beepr::beep() # ~1 hr 15 mins w 10 cores
...
pdp::partial(cf_no_dins               # the model
 ...
 stopCluster(cl); toc(); beepr::beep() # ~1 hr 13 mins
...
pdp::partial(cf_no_dins               # the model
 ...
 stopCluster(cl); toc(); beepr::beep() # ~1 hr 18 mins
...
pdp::partial(cf_no_dins               # the model
 ...
 stopCluster(cl); toc(); beepr::beep() # ~1 hr 20 mins
...
pdp::partial(cf_no_dins               # the model
 ...
 stopCluster(cl); toc(); beepr::beep() # ~1 hr 45 mins
...
pdp::partial(cf_dins_binary               # the model
  ...
  as_tibble() |> 
  group_by(build_p_otherpaved_200) |> 
  summarise(ci = list(mean_cl_normal(yhat))) |> 
  unnest(ci) |> 
  rename(x = build_p_otherpaved_200) |> 
  mutate(iv = 'build_p_otherpaved_200') |> 
  write_csv(paste0(getwd(), '/output_data/pdp_dins/build_p_otherpaved_200_', Sys.Date(), '.csv'))
...
(
  pdp_cf_no_dins_tibble <- tibble(files = dir('output_data/pdp', full.names = TRUE)) %>%
    ...
    select(-files) |> 
    left_join(variable_importance |> distinct(iv = Variable, Alias), by = 'iv') |> 
...
(
  pdp_cf_no_dins_tibble <- tibble(files = dir('output_data/pdp_dins', full.names = TRUE)) %>%
...
pdp_cf_no_dins_tibble |> tabyl(iv)
pdp_cf_no_dins_tibble |> tabyl(Alias, iv)
# pdp_cf_no_dins_tibble |> tabyl(x)
...
(
  b_build_WINDOWPANE <- pdp_cf_no_dins_tibble |>
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5)) +
    facet_wrap(~str_wrap(Alias, 30), scales = 'free_x') +
    labs(y = 'Predicted Probability', x = '(Multi Pane as reference)') + 
    NULL
  )

(
  c_build_EAVES <- pdp_cf_no_dins_tibble |>
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5)) +
    facet_wrap(~str_wrap(Alias, 30), scales = 'free_x') +
    labs(y = 'Predicted Probability', x = '(Enclosed as reference)') + 
    NULL
  )

(
  d_build_near_dest <- pdp_cf_no_dins_tibble |>
    ...
    theme_bw(16) +
    facet_wrap(~str_wrap(Alias, 30), scales = 'free_x') +
    labs(y = 'Predicted Probability', x = 'Distance') + 
    NULL
  )


(
  e_build_p_otherpaved_200 <- pdp_cf_no_dins_tibble |>
    ...
    theme_bw(16) +
    facet_wrap(~str_wrap(Alias, 30), scales = 'free_x') +
    labs(y = 'Predicted Probability', x = '% cover') + 
    NULL
  )
...
(
  f_build_VENTSCREEN <- pdp_cf_no_dins_tibble |>
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5)) +
    facet_wrap(~str_wrap(Alias, 30), scales = 'free_x') +
    labs(y = 'Predicted Probability', x = 'Vent Screen') + 
    NULL
  )
...
(
  g_build_min_dist_shrub <- pdp_cf_no_dins_tibble |>
