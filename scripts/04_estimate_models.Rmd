...
# keep only the building IDs and join to final data layer used for statistical analyses and models
...
#export into output_data
names(build_gis2)

# Dexter, is there a good way to export this as a shapefile / into a geodatabase, and retain field names as formatted?
# the below gives a lot of warnings and seems to be truncating field names
build_gis2 %>% 
  st_write(., paste0(getwd(), '/output_data/build_gis_ASyphard_', Sys.Date(), '.shp'))
...
#can either export parcel_gis into output_data, or pull out in ArcCatalog
#Dexter see note above, if there's a best way to do this in R and have a record, would be great.
names(build_gis2)
...
#assumes already read in tab_labs2 <- 
#read_csv(paste0(getwd(), '/data/Table_Deviance_Explained_2022-06-13.csv'))
  ...
  group_by (Model) %>%
   filter (rank<=top_n) %>% 
    filter(Model!='cf_dins_binary') %>% 
      tidylog::select(Variable, Model, Alias, Type)  -> top_pdp
