...
tic(); glm_spatial_diag <- 
  ...
  mutate(  data        = map(data,  ~drop_na(.x))
         ...
         , nbs         = map(data,  ~sfdep::st_knn(sf::st_geometry(.x), 5)), #Feb 14 2023 doesn't run - no applicable method for 'st_geometry' applied to an object of class "c('tbl_df', 'tbl', 'data.frame')"
