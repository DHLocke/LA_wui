...
# data?
# 2/23/22 Dexter, these are expected. Please see Script 2, section ## 4 DINS data and subsection a, exploring DINS data
# Dexter, we should keep the NAs in DINS data and find a random forest model that can make use of missing
# data
build_no_dins %>% 
  map(~sum(is.na(.))) %>% 
  bind_rows() %>% 
...
#  2/23/22 Dexter, since I would like to use party::cforest for DINS analyses below, due to NAs, should we use keep NAs here and use party::cforest as well? You prefer to drop these and use the different forest?
# random forest 
