...
tic(); load(file = "../Massive_forests/build_a_df_withoutdins.Rdata"); toc() # ~40 seconds (DHL: 0.034 seconds something not right)
...
#Dexter - does line 482 need to be taking in my varimp work above?? Lines 477-502 don't seem to be working for me as intended.
# Miranda, I dont understand. On 472 we calculated variable importance, we don't need to re-calculate it in 481. 
...
# } 
# 
# #Dexter - I still can't get the below to run so I make a graph. I need something created for cforest_imporatnce, and line 487 makes a figure but saves nothing in cforest_importance for me
...
(
  ...
  varimp_withoutdins |> 
    ...
    rename('Variable Importance' = varimp_withoutdins) %>% 
    left_join(
      tab_labs %>% # Miranda: it looks like with the new distance field names from Arc, the tab_labs needs to be comparably updated
...
# I (DHL) think this is done above. 
...
# MIRANDA: edarf isn't available for my version of R, does it work for you?
