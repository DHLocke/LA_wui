...
## 1 read in the data
```{r}
build <- read_csv(paste0(getwd(), '/output_data/building_2021-04-12.csv')) %>% 
...
tic(); uni_log_mods <- build %>%
  ...
  mutate(
    ...
    log_build_p_dist_orig_0= map(., ~glm(damage_binary ~ build_dist_to_build_ft,family = binomial, data = data.frame(.))),
    log_build_build_jod= map(., ~glm(damage_binary ~ build_dist_to_build_ft_jod,family = binomial, data = data.frame(.)))
...
#fitting model as suggested by GOF
log_maximal<- glm(damage_binary ~ build_Mean_aspect_100m_DEM+build_p_building_300+build_Mean_elev_30m+build_p_grass_300+
                   build_p_otherpaved_300+build_p_road_300+build_p_shrub_300+build_Mean_slope_100m_DEM+build_p_soil_200+
                    build_p_tree_100+build_p_water_100, family = binomial, data = build)


plot_model(log_maximal, type="std")

plot_model(log_maximal) + ylim(.75,1.25)
