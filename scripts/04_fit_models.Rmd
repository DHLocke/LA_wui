...
tic();test <- predict(forest, OOB = TRUE);toc() #with 100 trees, time is 180 sec, ~20 mins with 2000 trees
...
blob<-with(build_a,table(damage_binary,predtrue))
blob
...
tic(); figRF <- cforestImpPlot(forest);toc() # ~165 seconds with 100 trees, takes 50 min with 2000 trees, DHL: 30 mins with 2000
...
# graph importance by type and scale - faceted, top n
top_n <- 20
(cforest_importance_to_graph |> 
  slice(1:top_n) |> 
  ggplot(aes(  x = `Variable Importance`
...
tic(); figRF_20 <- cforestImpPlot20(forest);toc() # ~165 seconds with 100 trees, takes 50 min with 2000 trees
...
forest %>% 
  ...
  plotPartial(smooth = FALSE, lwd = 2, ylab = "Prob. of Destruction", xlim=c(0,150),ylim=c(0,0.5), xlab="Axis label")
