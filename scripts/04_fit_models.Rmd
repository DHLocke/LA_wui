...
# # this is a classification tree, random forest below
# tic()
...
build_a$predtrue <- build_a$predfor>0.340 # TODO figureout where this threshold comes from
blob<-with(build_a,table(damage_binary,predtrue))
blob
100*(blob[1]+blob[4])/sum(blob) # 89% accurate!!!
...
tic(); figRF <- cforestImpPlot(forest);toc() # ~95 seconds with 100 trees, takes 50 min with 2000 trees

# TODO? do this in ggplot with colors or other symbols corrisponding to framework categories.
...
figRF %>% ggsave(file = paste0(getwd(), '/figures/var_impt_plot_',
