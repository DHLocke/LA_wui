...
tic(); uni_log_mods <- build %>%
  ...
  nest(data=everything()) %>% #Dexter - this doesn't work for me, 2/2/22
  # nest()%>%
  mutate(
    # ~landscape position
...
forest<-binary_without_dins_cols; rm(binary_without_dins_cols) # keeping Owen's naming conventions
 ...
 tic();preds <- predict(forest, OOB = TRUE);toc() #with 100 trees, time is 180 sec, ~49 mins with 2000 trees

 # type change..
 build_a_df <- build_a  %>% data.frame()
 tic(); build_a_df$predfor = predict(forest, OOB = TRUE); toc() #41 min
 build_a_df$predtrue <- build_a_df$predfor>0.1170289 #TODO Add in work to get 0.117 
...
# with 2000 trees,the model had 76% accuracy with 23.1% false negative and 0.9% false positive rate - just checking is it ok that dataset unbalanced?
...
#Dexter this isn't saving for me :((((, why?! I can run the code and generate hte figure but cforest_importance is empty
...
# MHM takes 3 min with 100 trees, settings here from Owen
...
build_a_df$predtrue<-build_a_df$predfor>0.1170289
...
round(100*(blob[3]/sum(blob)),1) #false negative rate
round(100*(blob[2]/sum(blob)),1) #false pos rate
# with 100 trees,the model had 77.4% accuracy with 22.2% false negative and 0.4% false positive rate - just checking is it ok that dataset unbalanced?
