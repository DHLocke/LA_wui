...
(tab_labs <- readxl::read_xlsx('data/Data LA Study 20211208.xlsx') %>% 
  ...
  tidylog::select(Type, `Variable name`, Deviance = dev2, n = n2,
         Scale, Source) %>% 
  arrange(Type, desc(Deviance)))

#looking at deviance table for univariate
head(tab_labs)
tab_labs %>% filter(Deviance > 5) %>%  mutate_if(is.double, round, 3) #top 5
