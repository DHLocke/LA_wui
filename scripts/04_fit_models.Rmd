...
This assumes "02_Woosley_combine_data.Rmd" as well as "02.1_DINS.Rmd" were run
...
build <- read_csv(paste0(getwd(), '/output_data/building_2021-11-16v2.csv')) %>% glimpse
```
...
(
  gof <- uni_log_mods %>% 
    ...
    unnest(everything()) %>% 
    unnest(everything()) #TODO doesn't run for me
)


# which distance provides the best fit per variable?
# in the case of build_near_dest and build_dist_to_build_ft_jod, pick between distroyed or not
(
  best_dist <- gof %>% 
    group_by(var) %>% 
...
# Miranda what is *this* near dist then and should we add it above? What are "build_near_dest" and 
# "build_near_exp" and are these rundundant with the other distnance measure or not?
# distance to destroyed
...
log_build_reg_data <- build %>%
  tidylog::select(damage_binary
                  ...
                  , parcel_Perv_P
                  , parcel_Imperv_P
                  , parcel_year_built # Dexter, shld this be parcel_recent?
                  # Miranda: this is my question to you? We can change it here, but this is what
                  # should be done in script 02.
                  # What comes into 03 and 04 should require no more modification. You tell me which
                  # vars to use. Also, we can rename them in 02. Straighten this out on line 634 in script 02
                  # , parcel_recent
                  , parcel_area,
  ...
  #drop_na() #now keeping NAs bc of Part
...
## PUNCHING IN Owen Price-inspired code
```{r}
...
#Dexter - here here here
# we want to calc deviance for each univariate model; Owen Price's code is on line 356 and I can't get it to work.
# did a few cals 'by hand' to look at 
mod <- glm(damage_binary ~ build_Mean_elev_30m, data = build, family = binomial())
100 * (mod$null.deviance - mod$deviance) / mod$null.deviance

mod <- glm(damage_binary ~ build_near_dest, data = build, family = binomial())
100 * (mod$null.deviance - mod$deviance) / mod$null.deviance

mod <- glm(damage_binary ~ build_p_soil_300, data = build, family = binomial())
100 * (mod$null.deviance - mod$deviance) / mod$null.deviance


#tried to use Owen Price's code, can't get this to work. #Dexter - here here here
#table1 contains a list of predictor variables for analysis and vectors to store the results for each variable 

# DHL here, I don't like this because it is immediately out of date
# IF 02's final export was done right the we would just do all variables but the dependent
# its another piece to go wrong. If anything changes, it needs to change instead of just pulling from
# the final analysis-ready data. Seem to invite error, hence the grumbles. And we keep having the same
# problem with the columns.. so I want to cut that off at the pass.
(table1 <- read_csv('data/dep_var_20211109.csv') %>% #created a list of predictor analyses as a csv
  mutate(dev2 = 0, n2 = 0) %>% 
    data.frame())

# table1$dev2<-0 # go ahead and delete now, see how its done in the import step?
# table1$n2<-0

build %>% dplyr::select(any_of(table1$upvar)) # lots of character variables in the DINS vars
build %>% glimpse                             # which could be the source of the error

# find the numeric columns to test that theory
build %>% dplyr::select_if(is_numeric) %>% 
  dplyr::select(damage_binary, any_of(table1$upvar)) %>% # so first numeric, then what was in your list
  data.frame() -> test_data                              # tibbles can be problematic in loops

dat.model <- data.frame(y = test_data$damage_binary, x = 0)
i<-0
for (varname in table1$upvar) {
  i<-i+1
  if (varname %in% colnames(test_data)) {
    dat.model$x <- test_data[, varname]
    mod <- glm(y ~ x, data = dat.model, family = binomial())
  } else {
    warning(varname, " not found in test_data", immediate. = TRUE)
    mod <- NULL
  }
  
  if (!is.null(mod)) {
    table1$n2[i] <- nrow(test_data) - sum(is.na(dat.model$x))
    table1$dev2[i] <- 100 * (mod$null.deviance - mod$deviance) / mod$null.deviance
  }
}

# Indeed, characters as predictors causes the failur. Maybe we should recode as factors? Do we want to
# test those predictors in Owen's deviance test?
# I (DHL) don't understand the purpose of this step at all.
...
#Dexter - here here here - if you want you can check out some trees I ran, here thru line 413
# next step after that is to build Random Forests

library(rpart) 
library(pROC) #for AUC calculation

#create dataset with only table1 predictor variables in it
table1 <- read.csv('data/dep_var_20211109.csv', stringsAsFactors = FALSE)  #created a list of predictor analyses as a csv
test<-build %>% 
  tidylog::select()#only the variables listed in table1, not sure how to do that?
                  # DHL the new code above does exaclty that. BUT we would't have to if the final block
                  # in script 02 did this. Sorry to sound like a broken recored: the output of 02 does not match what is bing called for here, and earlier, and later. Rather than re-write each of those sections, lets 'stick the landing' 10/10 on the last section of 02.

rfdat<-build %>% #manually selected variables, not ideal
  # wouldn't have to.....
  tidylog::select(build_Mean_elev_30m:build_Mean_distall_road,
                  damage_binary,
                  build_area:build_near_dest,
                  parcel_Can_P:parcel_Imperv_P,
                  parcel_area:build_DINS)

# DHL ok fine here, but just this once be
rfdat <- build %>% 
  tidylog::select(damage_binary, any_of(table1$upvar))

# rfdat1<-rfdat%>% #dropping the binary value that indicates in DINS
#   tidylog::select(!build_DINS)

#random forest for full data set
tic(); final <- rpart(damage_binary ~ ., data = #rfdat1
                        rfdat, method = 'anova'); toc()
plot(final)
summary(final) # 
plot(final, branch=0.5); text(final,cex=0.7)

# DHL ended here.

dinsonly<-rfdat%>% filter(build_DINS==1) # but this column is dropped manually above. 
tic(); final <- rpart(damage_binary ~ ., data = dinsonly, method = 'anova'); toc()
plot(final)
summary(final) #1534 observations
plot(final, branch=0.5); text(final,cex=0.7)
# Owen Price
# windows(4.5,4.5)
# par(mar=c(1,1,2,1))
...
windows(4.5,4.5)
par(mar=c(1,1,2,1))
maximal_rpart<-prune(final,cp=0.02)  
plot(maximal_rpart, branch=0.5); text(maximal_rpart,cex=0.7)
printcp(maximal_rpart)
print(maximal_rpart)
...
### I fit Jeffrey Evan's worked example: randomForest
```{r}
