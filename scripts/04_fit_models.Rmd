...
tic(); uni_log_mods <- build %>%
  ...
  mutate(
    ...
    log_build_near_dest= map(., ~glm(damage_binary ~ build_near_dest,family = binomial, data = data.frame(.))), #Dexter this and line below are still giving errors but these variables are in data. not sure what's up; maybe issues is build_dist_to_build_ft_jod has some NAs?
    log_build_dist_to_build_ft_jod= map(., ~glm(damage_binary ~ build_dist_to_build_ft_jod,family = binomial, data = data.frame(.)))
    ) %>%
  dplyr::select(-data) %>% 
  rowid_to_column() %>% 
...
table1 <- read.csv('data/dep_var_20211109.csv', stringsAsFactors = FALSE)  %>% glimpse #created a list of predictor analyses as a csv
...
tic(); final <- rpart(damage_binary ~ ., data = rfdat1, method = 'anova'); toc()
plot(final)
summary(final) # 
plot(final, branch=0.5); text(final,cex=0.7)

dinsonly<-rfdat%>% filter(build_DINS==1)
