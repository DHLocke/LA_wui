...
#DEXTER - is rpart actual random forest? or is it classification tree (one?) 
tic()
...
library(party) #if we keep move up
tic()
binary_without_dins_cols <-cforest(damage_binary ~ ., 
                                  data = build_a %>% 
                                    tidylog::select( # dropping columns containing specific DINS data
                                            # very detailed but sparse (lots of NAs)
                                      !c(  build_DECKPORCHO
                                  ...
                                  ,controls=cforest_control(mtry=5, ntree=2000, mincriterion=0)); toc() #takes 12 min, copied his settings

#from owen
forest<-binary_without_dins_cols
build_a$predfor <- predict(forest, OOB = TRUE) 
build_a$predtrue <- build_a$predfor>0.340
blob<-with(build_a,table(damage_binary,predtrue))
blob
100*(blob[1]+blob[4])/sum(blob)
tic();varimp(forest);toc() #takes 84 min! 
...
tic();figRF<-cforestImpPlot(forest);toc() #takes 50 min
