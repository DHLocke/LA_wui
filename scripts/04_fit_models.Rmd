...
#commented out this work and read in table of deviance below
...
# # TODO drop 'parcel_Perv_P' here or above; same with build_DINS
...
# reading in table of deviance explained, work done and commented out above
(tab_labs <- 
  read_csv(paste0(getwd(), '/data/Table_XX_Deviance_Explained.csv')))

#next steps for getting table to look nice  type look up table, add alias name for reporting - is now in Data LA Study 20211208.xlsx
...
#                                          , build_WINDOWPANE
#                                     ))
#                                   ,controls=cforest_control(mtry=5, ntree = n_tree, mincriterion=0)); toc() #takes 12 min with full data set, settings here from Owen (DHL 7 mins with n_tree = 2000 )
...
tic(); load(file="../Massive_forests/binary_without_dins_cols.Rdata"); toc() # ~40 seconds

# from owen
forest<-binary_without_dins_cols; rm(binary_without_dins_cols) # keeping Owen's naming conventions

# tic();preds <- predict(forest, OOB = TRUE);toc() #with 100 trees, time is 180 sec, ~20 mins with 2000 trees

# # type change..
# build_a_df <- build_a |> data.frame()
# tic(); build_a_df$predfor = predict(forest, OOB = TRUE); toc() 

# tic(); save(build_a_df, file="../Massive_forests/build_a_df.Rdata"); toc()

tic(); load(file="../Massive_forests/build_a_df.Rdata"); toc() # ~40 seconds
...
# # trying to figure out a binary (0,1) prediction
# tic();build_a$predfor <- predict(forest, type='prob', OOB = TRUE);toc() #with 100 trees, time is 180 sec

# preds |> hist()
# preds |> summary()

# build_a$predtrue <- test > .34 # TODO figureout where this threshold comes from
# build_a$predtrue <- build_a$predfor>0.34 # TODO figureout where this threshold comes from
#Dexter, I'm not having much luck finding this threshold. Is it just going from continuous predictor to binary outcome?
#some potential relevant information online -  
#https://cran.r-project.org/web/packages/party/party.pdf (pages 5 -7)
#https://stats.stackexchange.com/questions/77290/does-party-package-in-r-provide-out-of-bag-estimates-of-error-for-random-forest
...
# with 100 trees,the model had 89.9% accuracy with 2.5% false negative and 7.6% false positive rate - just checking is it ok that dataset unbalanced?

# DHL 23 mins 
# tic();varimp(forest);toc() # ~90 seconds with 100 trees, takes 84 min with 2000 trees! 

#plot important variables
#from Owen and ?https://favorableoutcomes.wordpress.com/2014/12/19/r-function-for-plotting-variable-importance-results-from-cforest-imlementation-of-random-forest-model/ ?
...
# tic(); cforest_importance <- cforestImpPlot(forest);toc() # ~165 seconds with 100 trees, takes 50 min with 2000 trees, DHL: 30 mins with 2000
# 
...
# TODO--getinto ggplot with colors or other symbols corrisponding to framework categories.
...
#### ii binary outcome for just DINS-surveyed 
```{r} 
...
#n_tree <- 2000 #use this for final analyses
n_tree <- 100 #testing number of trees
#NOTE DHL - i need to remove build_DINS from the build_a dataset.
...
#MHM takes 2 min with 100 trees, settings here from Owen (DHL 7 mins with n_tree = 2000 )

# #when finalized use this to save this Random Forest 
# tic(); save(binary_with_dins_cols, file="../Massive_forests/binary_with_dins_cols.Rdata"); toc()

# #when finalized use this to load Random Forest
#tic(); load(file="../Massive_forests/binary_with_dins_cols.Rdata"); toc() # ~40 seconds
...
build_a_df <- build_a  %>%  data.frame()
tic(); build_a_df$predfor = predict(forest, OOB = TRUE); toc()
build_a_df$predtrue<-build_a_df$predfor>0.340
# #change names tic(); save(build_a_df, file="../Massive_forests/build_a_df.Rdata"); toc()
#change names #tic(); load(file="../Massive_forests/build_a_df.Rdata"); toc() # ~40 seconds
...
round(100*(blob[3]/sum(blob)),1) #false negative rate
round(100*(blob[2]/sum(blob)),1) #false pos rate
# with 100 trees,the model had 91.8% accuracy with 2% false negative and 6% false positive rate - just checking is it ok that dataset unbalanced?

# 
 tic();varimp(forest);toc() # ~4 min with 100 trees, takes 84 min with 2000 trees! 
...
cforestImpPlot <- function(x) {
 ...
 tic(); plotsave <- cforestImpPlot(forest);toc() # ~362 seconds with 100 trees, takes XX min with 2000 trees 
# 
# #need to change name tic(); save(cforest_importance, file="../Massive_forests/cforest_importance"); toc()

#ned to change name tic(); load(file="../Massive_forests/cforest_importance.Rdata"); toc()
