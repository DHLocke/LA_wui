...
#followed Owen Price code
table1 <- read.csv('data/dep_var_20211109.csv', stringsAsFactors = FALSE)  
#combdat<-read.csv("bia_anal_2020.csv") is my build
...
tic(); final <- rpart(damage_binary ~ ., data = log_build_reg_data, method = 'anova'); toc()
plot(final)
summary(final) #7537 observations
plot(final, branch=0.5); text(final,cex=0.7)

dinsonly<-log_build_reg_data %>% filter(build_DINS==1)
