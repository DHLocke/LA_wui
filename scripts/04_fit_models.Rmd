...
build_a <- build %>% #retain only the 72 predictors of interest for final analyses and damage_binary as response 
...
mod <- glm(damage_binary ~ build_Mean_elev_30m, data = build_a, family = binomial())
100 * (mod$null.deviance - mod$deviance) / mod$null.deviance

mod <- glm(damage_binary ~ build_near_dest, data = build_a, family = binomial())
100 * (mod$null.deviance - mod$deviance) / mod$null.deviance

mod <- glm(damage_binary ~ build_p_soil_300, data = build_a, family = binomial())
100 * (mod$null.deviance - mod$deviance) / mod$null.deviance
...
table1 <- list(names(build_a)) %>% #created a list of predictors
   data.frame()%>% 
    mutate(dev2 = 0, n2 = 0) %>%
   rename(var=c..damage_binary....build_area....build_DECKPORCHE....build_DECKPORCHO...)%>%
     data.frame()

build_a %>% dplyr::select(any_of(table1$upvar)) # lots of character variables in the DINS vars
build_a %>% glimpse                             # which could be the source of the error

# find the numeric columns to test that theory
build_a %>% dplyr::select_if(is_numeric) %>% 
