...
library(rpart) #DEXTER, do you want this to go elsewhere?
library(pROC) #for AUC calculation

#build - full dataset - 11181 obs, 85 cols
#build_a - all variables you want as predictor for random forest - 11,181 obs, 72 cols

#build_dins- dataset without DINS variables - 11181 obs, 60 vars;
#DEXTER alternatively if we need this can just exclude these cols from random forest model?
build_nodins <- build_a %>% tidylog::select(!c(build_DECKPORCHE,
            build_DECKPORCHO    ,
                 build_EAVES    ,
            build_EXTERIORSI    ,
            build_FENCEATTAC    ,
...
#build_dins -  dataset just of DINS data - 1534 obs, 72 cols
build_dins <- build %>% filter(build_DINS==1)%>% 
  tidylog::select(any_of(table1$upvar))

 
#RF1 random forest for DINS data set
tic(); dins <- rpart(damage_binary ~ ., data = build_dins,
                        method = 'anova'); toc()
plot(dins); text(dins,cex=0.7)
summary(dins)  

#code from Owen
windows(4.5,4.5)
par(mar=c(1,1,2,1))
dins1<-prune(dins,cp=0.02)
plot(dins1,branch=0.5);text(dins1,cex=0.7)
printcp(dins1)
print(dins1)


#RF2 random forest for full data, includes DINS - when you do this, none of DINS variables are in top branches. 
#not sure if it should include DINS or drop those variables. would be 11181 obs still
tic(); final <- rpart(damage_binary ~ ., data = build_a, method = 'anova'); toc()
plot(final)
summary(final) # 11181  observations
...
#RF3 random forest forall obs, drop DINS
tic(); final <- rpart(damage_binary ~ ., data = build_nodins, method = 'anova'); toc()
plot(final)
summary(final) # 11181  observations
plot(final, branch=0.5); text(final,cex=0.7)
