...
tic(); uni_log_mods <- build %>%
  ...
  nest(data=everything()) %>% #DEXTER - this gives me an error "`everything()` must be used within a *selecting* function."
  # nest()%>%
  mutate(
    # ~landscape position
...
#Dexter, since you don't like bringing in a csv with the variables of interest I did it below in code. Does this work for you?

build_a <- build %>% #retain only the 72 variables of interest for final analyses; go from 96 in build to 72
  tidylog::select(damage_binary,
                  build_area    ,
            build_DECKPORCHE    ,
            build_DECKPORCHO    ,
...
# we want to calc deviance for each univariate model;
