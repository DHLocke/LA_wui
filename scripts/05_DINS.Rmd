...
#3.1 DECKPORCHE deck/porch materials #change 'no deck/porch' to N/A 
#4. roof materials
#5. patio/cover materials #reclass 'no' to N/A
#6. propane tank distance #reclass categories so blank is unknown and others are in order, N/A = no   tank
#7. window panes #reclass No windows to NA
#8. vegetation clearance #VEGCLEARAN #reclass missing to Unknown
...
dins %>% 
 st_drop_geometry() %>% 
  filter(!is.na(Buildings_ParcelID_DINS_intersects_INCIDENTST)) %>%
 tabyl(Buildings_ParcelID_DINS_intersects_DECKPORCHO) #deck and porch --not sure what E is vs O    
...
#12? #don't know what to do with blank fields. don't use unless confirm
dins %>% 
  st_drop_geometry() %>% 
  filter(!is.na(Buildings_ParcelID_DINS_intersects_INCIDENTST)) %>%
